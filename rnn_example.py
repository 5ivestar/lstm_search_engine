...
if __name__ == '__main__':
  ...
  cell = tf.contrib.rnn.BasicLSTMCell(size, seq_width)  
  initial_state = cell.zero_state(batch_size, tf.float32)

  # ========= This is the most important part ==========
  # output will be of length 4 and 6
  # the state is the final state at termination (stopped at step 4 and 6)  
  outputs, state = tf.contrib.rnn.static_rnn(cell, inputs, initial_state=initial_state, sequence_length=early_stop)
  ...
  pritn("outputs, should be 2 things one of length 4 and other of 6")
  outs = session.run(outputs, feed_dict=feed)
  print(outs)
  for i,xx in enumerate(outs):
    print(i,xx)
