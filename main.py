...
querys,documents,urls=data_handler.make_training_data()
print(len(querys),len(documents),list(vector_cache.keys())[:10])

doc_size=len(documents)
training_model=lstm_model.LstmSearchModel(doc_size,mode="training",scope_reuse=True)
...
qp_model=lstm_model.LstmSearchModel(doc_size,mode="query_processing",doc_vecs=doc_vecs,scope_reuse=True)
